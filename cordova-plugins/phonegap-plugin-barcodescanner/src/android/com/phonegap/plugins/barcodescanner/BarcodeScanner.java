...
public class BarcodeScanner extends CordovaPlugin {
    ...
    private static final String SAVEBARCODE = "saveBarcode";
    ...
    private static final String B64IMAGE = "b64Image";
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) {
        ...
        if (action.equals(ENCODE)) {
            ...
            JSONObject obj = args.optJSONObject(0);
            if (obj != null) {
                String b64Image = obj.optString(B64IMAGE);

                if (b64Image == null) {
                    callbackContext.error("User did not specify b64image to save");
                    return true;
                }

                saveBarcode(b64Image);
            } else {
                callbackContext.error("User did not specify data to save");
                return true;
            }
        } else {
            return false;
    ...
    public void saveBarcode(final String b64Image) {
      new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                  File dataDir = new File(Environment.getExternalStorageDirectory(), "cloneboxassistant");
                  if (!dataDir.exists()) {
                      dataDir.mkdirs();
                  }

                  File file = new File(dataDir, "barcode.png");
                  Log.i("##RDBG", "filepath:" + file.getAbsolutePath());
                  ...
                  fOut.close();
                  Log.i("##RDBG", "decode suc:" + file.getAbsolutePath());
                  callbackContext.success(file.getAbsolutePath());
                }
                catch (Exception e) {
                    Log.e("##RDBG", "saveBardoce failed, exception: " + e);
                }
            }
        }).start();
    }
