...
- (void)saveBarcode:(CDVInvokedUrlCommand*)command;
...
- (void)saveBarcode:(CDVInvokedUrlCommand*)command {
    ...
    NSString *base64ImgStr = command.arguments[0][@"b64Image"];
    ...
    int i = 1;
    do {
        filePath = [NSString stringWithFormat:@"%@/%@%03d.%@", docsPath, @"barcode", i++, @"png"];
    } while ([fileMgr fileExistsAtPath:filePath]);
    
    if (![imgData writeToFile:filePath options:NSAtomicWrite error:&err]) {
        [self returnError:[NSString stringWithFormat:@"saveBardoce failed, error:%@",[err localizedDescription]] callback:command.callbackId];
    } else {
        
        NSLog(@"filePath :%@", [[NSURL fileURLWithPath:filePath] absoluteString] );
        
        [self returnSuccess:[[NSURL fileURLWithPath:filePath] absoluteString] callback:command.callbackId];
    }
...
-(void)getInfoWithImage{
    ...
    if (resultString) {
        ...
        NSLog(@"ç¸ĺĺžçcontents == %@",resultString);
        ...
        [self.plugin returnError:@"č§Łćĺ¤ąč´Ľ" callback:self.callback];
